...
import com.aoshi.util.SmsUtil;
import com.aoshi.util.Tools;
import com.aoshi.util.Watermark;
import com.aoshi.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public Object getList() {
		...
		try {
			...
			if (null == pds) {
				...
				pds = userService.findByUId(session.getAttribute(Const.SESSION_USER));
				if (pds.getString("as_shop_id") != null) {
					AsShop shop = (AsShop) asShopMapper.selectByPrimaryKey(pds.getInt("as_shop_id"));
					pds.put("shop", shop);
				}
	 ...
	 * 去代码生成器页面
	 */
	@RequestMapping(value = "/goProductCode_auto")
	public ModelAndView goProductCode_auto() throws Exception {
		ModelAndView mv = this.getModelAndView();
		List<String> tlist = headService.getTable();
		mv.addObject("tlist", tlist);
		mv.setViewName("system/head/productCode_auto");
		return mv;
	}
