...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="shopowner/listAll.do" method="post" name="usersForm" id="usersForm">
				...
				<table>
							<tr>
								<td><span class="input-icon"> <input
										...
										name="keyWord" value="${pd.keyWord}" placeholder="商家帐号/商家名称/商家电话" style="width:400px;"/>
					...
					<c:when test="${not empty list}">
						<c:forEach items="${list}" var="var" varStatus="vs">
							...
							<tr>
								...
								<td class='center' style="text-align:center;">${var.contact}</td>
								<td class='center' style="text-align:center;">${var.tel}</td>
								<td class='center' style="text-align:center;">
								<c:if test="${var.ownerStatus == 0}">
									<a href="javascript:void(0);" onclick="accountStatus(${var.userId},1)">禁用</a>
								</c:if>
								<c:if test="${var.ownerStatus == 1}">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function edit(user_id){
			...
			form.find('#showCount').attr('name','showCount');
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>shopowner/page.do?action=2&userId='+user_id;
			 diag.Width = 700;
			 diag.Height = 600;
		...
		function query(user_id){
			...
			form.find('#showCount').attr('name','showCount');
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>shopowner/page.do?action=4&userId='+user_id;
			 diag.Width = 700;
			 diag.Height = 600;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function resetPwd(user_id){
			...
			form.find('#showCount').attr('name','showCount');
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>shopowner/resetPwdPage.do?userId='+user_id;
			 diag.Width = 500;
			 diag.Height = 600;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function accountStatus(userId,status){
			...
			form.find('#currentPage').attr('name','currentPage');
			form.find('#showCount').attr('name','showCount');
			$.get("shopowner/accountStatus",{userId:userId,status:status},function(data){
				...
				console.log($('#showCount').attr('name'))
				alert("操作成功")
				form.submit();
			},'json');
		}
		...
		function idelete(userId){
			...
			form.find('#currentPage').attr('name','currentPage');
			form.find('#showCount').attr('name','showCount');
			$.get("shopowner/delete",{userId:userId},function(data){
