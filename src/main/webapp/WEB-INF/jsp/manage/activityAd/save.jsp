...
<body>
	...
	<input type="hidden" name="adPosType" id="adPosType" value="${adPosType}">
	...
	<%-- <c:if test="${action != null && action !=1 }">
		...
		<tr height="45px">
			...
			<td>广告板块类型:</td>
			<td>
              <select style="width: 370px"  onchange="selectActivity(this)"<c:if test="${adPosType !=null || adPosType != ''}">disabled="disabled"</c:if> <c:if test="${action !=null && action != '' && action==4 }">disabled="disabled"</c:if>>
                  <option value="1" <c:if test="${pd.adPosType==1 ||adPosType==1}">selected="selected"</c:if> >E街E铺</option>
                  <option value="3" <c:if test="${pd.adPosType==3 ||adPosType==3}">selected="selected"</c:if>>积分商城</option>
              </select>
          </td>
		</tr>
	...
	<tr height="45px">
			<td>广告url类型:</td>
			<td>
              <select style="width: 370px" disabled="disabled">
		...
		<tr>
            ...
            <c:if test="${pd == null || pd.adImg== '' || pd.adImg == null }">
              ...
              <span style="color: red; width: 50px;" id="msgDivScreen" >注： 图片格式支持 : gif/png/jpg/jpeg  尺寸规格 1188 * 1022</span>
            </c:if>
            <c:if test="${pd != null && pd.adImg != '' && pd.adImg != null }">
