  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<input type="hidden" name="activityType" value="${activityType}"/>
				<input type="hidden" name="adUrlType" value="${adUrlType}"/>
								...
								<c:if test="${var.adUrlType==1}">
								 ...
								 <c:if test="${var.adUrlType==1}">
									 <c:if test="${var.adPosType==2}">
										 <td class='center' style="text-align:center;">大转盘-领取页</td>
									 </c:if>
									 <c:if test="${var.adPosType==6}">
										 <td class='center' style="text-align:center;">大转盘-logo</td>
									 </c:if>
									 <c:if test="${var.adPosType==7}">
										 <td class='center' style="text-align:center;">大转盘-首页</td>
									 </c:if>
								 </c:if>
								 <c:if test="${var.adUrlType==0}">
									 <td class='center' style="text-align:center;">红包雨扫码</td>
								 </c:if>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(otype){
			...
			form.find('#showCount').attr('name','showCount');
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>activityAd/page.do?action='+otype+'&adPosType='+'${adPosType}'+'&adUrlType='+'${adUrlType}';
			 diag.Width = 950;
			 diag.Height = 650;
		...
		function edit(id){
			...
			form.find('#showCount').attr('name','showCount');
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>activityAd/page.do?action=2&adId='+id+'&otype='+1+'&adPosType='+'${adPosType}'+'&adUrlType='+'${adUrlType}';
			diag.Width = 950;
			diag.Height = 650;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function query(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>activityAd/page.do?action=4&adId='+id+'&adPosType='+'${adPosType}'+'&adUrlType='+'${adUrlType}';
			diag.Width = 950;
			diag.Height = 650;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
