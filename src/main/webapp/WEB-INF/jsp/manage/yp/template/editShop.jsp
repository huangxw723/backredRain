	...
	<div id="page-content" class="clearfix">
		...
		<div class="row-fluid">
			...
			<form class="form-horizontal" id="form" action="yp/admin/template/saveShop.do" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" name="moduleShopId" value="${shop.moduleShopId}">
				<div class="control-group" >
					...
					<div class="controls">
							...
							<span class="help-inline">
								...
								<c:if test="${shop.shopImage != null && shop.shopImage != ''}">
									<a href="javscript:void(0);" onclick="delShopImg()">删除</a>
								</c:if>
							</span>
							<div style="float: left;">
								<img id="image1" alt="" src="<%=httpUrl%>${shop.shopImage}" style="max-width:350px;"/>
							</div>
				...
				<div class="control-group" >
					...
					<div class="controls">
							...
							<span class="help-inline">
								...
								<c:if test="${shop.shopLogo != null && shop.shopLogo != '' }">
									<a href="javscript:void(0);" onclick="delShopLogo()">删除</a>
								</c:if>
							</span>
							<div style="float: left;">
								<img id="image2" alt="" src="<%=httpUrl%>${shop.shopLogo}" style="max-width:350px;"/>
							</div>
							<!-- <input id="goodsImg" type="hidden" name="goodsImg"> -->
					</div>
					<div class="controls">
		...
		function delShopImg(){
			if(confirm("确认删除店铺图片吗?")){
				var businessModuleId = ${businessModuleId};  
				$.ajax({
						type: "POST",
						url: '<%=basePath%>yp/admin/template/delShopImg.do',
				    	data: {businessModuleId:businessModuleId},
						dataType:'json',
						//beforeSend: validateData,
						cache: false,
						success: function(data){
							if(data){
								var dialog = top.Dialog._dialogArray[0];
								dialog.delCallback();
								history.go(0);
							}
						}
		...
		function delShopLogo(){
			if(confirm("确认删除店铺logo吗?")){
				var businessModuleId = ${businessModuleId};  
				$.ajax({
						type: "POST",
						url: '<%=basePath%>yp/admin/template/delShopLogo.do',
				    	data: {businessModuleId:businessModuleId},
						dataType:'json',
						//beforeSend: validateData,
						cache: false,
