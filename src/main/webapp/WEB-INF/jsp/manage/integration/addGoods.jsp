	...
	<meta charset="utf-8" />
		...
		function  setSalesPrice() {
			...
			function save(){
				...
				if($("#salesPrice").val() < 1000000|| $("#salesPrice").val() > 1 ){
					var pattern = new RegExp(/^[1-9][\d]{0,7}$/g);
					var b = pattern.test($("#salesPrice").val());
					if(!b){
						$("#salesPrice").tips({
							side:3,
				            msg:'请输入正整数',
				...
				var fileType1 = document.getElementById("fileInput1").value.substr(document.getElementById("fileInput1").value.lastIndexOf(".")).toLowerCase();//获得文件后缀名
				var url1 = $("#url1").val();
				var img1 = $("#imgHref1").attr("src");
				if (!img1 || url1) {
					if (fileType1 != '.gif' && fileType1 != '.png' && fileType1 != '.jpg'&& fileType1 != '.bmp'
							&& fileType1 != '.jpeg') {
						bootbox.alert("请上传图片格式的文件");
						$("#fileInput1").tips({
							side : 3,
				...
				var img = document.getElementById("fileInput1").files;
				var imgSize = img[0].size/1024;
				if(imgSize>2048){
					bootbox.alert("图片不能大于2M,请更换图片");
					$("#fileInput1").tips({
...
<body>
	 <div id="page-content" class="clearfix" >
		...
		<div class="row-fluid">
			...
			<form class="form-horizontal" id="form" action="goods/addGoods.do" method="post">

				<div class="control-group">
					<label class="control-label" >商品名称</label>
					<div class="controls">
						<textarea style="width:210px;height:60px;" id="goodsName"  name="goodsName" placeholder="2~50个字" required onblur="checkExist()" ></textarea>
					</div>
				</div>
				<div class="control-group">
					<label class="control-label">商品货号</label>
					<div class="controls">
						<input type="text" id="goodsSn" name="goodsSn"  required onblur="checkExistSn()"/>
					</div>
				</div>
				<div class="control-group">
					<label class="control-label">市场价</label>
					<div class="controls">
						<input type="number" id="marketPrice" name="marketPrice" min="0.01" max="99999.99" onchange="setSalesPrice();" placeholder="0.01~99999.99" required>
					</div>
				</div>
				<div class="control-group">
					<label class="control-label">兑换积分</label>
					<div class="controls" >
						<input type="number" id="salesPrice" name="salesPrice" min="1" max="1000000" value=""  placeholder="1~1000000" onkeyup="validateUtils.replaceNotNumEmpty(this);" >
					</div>
				</div>
				 <div class="control-group">
				...
				<div class="control-group">
					...
					<div class="controls">
						<input type="number" id="weight" name="weight" min="0.01" max="99.99" placeholder="0.01~99.99" required>
						单位(kg)
					</div>
				</div>
				 <div class="control-group">
				...
				<div class="control-group" >
					<label class="control-label" id="imgFile">商品主图</label>
					<div class="controls" style="width: 230px">
				              <a id="imgBox" href="javascript:void(0)" style="display:none;">
				                <img src="" style="width: 50px; height: 50px"  id="imgHref" />
				              </a>
				              <input type="file" id="fileInput" name="fileInput" onchange="selectImg(this)" />
				              <font color="red">注：(图片尺寸规格320px * 320px)</font>
				            <input id="url" name="img" type="hidden" />
					</div>
				</div>
				<div class="control-group" >
					<label class="control-label" id="imgFile1">商品图片</label>
					<div class="controls" style="width: 230px">
				              <a id="imgBox1" href="javascript:void(0)" style="display:none;">
				                <img src="" style="width: 80px; height: 50px"  id="imgHref1" />
				              </a>
				              <input type="file" id="fileInput1" name="fileInput1" onchange="selectImg1(this)" />
				...
				<div class="control-group">
					...
					<div class="controls">
						<textarea style="width:300px;height:40px;" id="remark" name="remark" ></textarea>
					</div>
				</div>
				<div class="control-group">
					...
					<div class="controls">
						...
						<textarea style="width:790px;height:259px;" name="goodsDesc" id="descp" placeholder="2~10000个字"></textarea>
					</div>
				</div>
