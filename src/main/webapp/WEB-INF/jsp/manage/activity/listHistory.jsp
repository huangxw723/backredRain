...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="acitity/listHistory.do" method="post" name="usersForm" id="usersForm">
			<input type="hidden" name="activityType" value="${pd.activityType}"/>
				<table>
					<tr>
								<td>
								<span class="input-icon"> 
									<input autocomplete="off" id="nav-search-input" type="text" name="keyWord" value="${pd.keyWord}" placeholder="活动名称/活动编号" />
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th style="text-align:center;">参与商家</th>
						<th style="text-align:center;">参与人数</th>
						<th style="text-align:center;">发放奖品(已发奖品数/总奖品数)</th>
					...
					<c:when test="${not empty list}">
						<c:forEach items="${list}" var="var" varStatus="vs">
							...
							<tr>
								...
								<td class='center' style="text-align:center;">${var.type}</td>
								<td class='center' style="text-align:center;">${var.shopCounts}</td>
								<td class='center' style="text-align:center;">${var.personCounts}</td>
								<td class='center' style="text-align:center;">${var.prizeCounts - var.prizeSurplusCounts}/${var.prizeCounts}</td>
								<td class='center' style="text-align:center;">${var.createTime}</td>
								
								<td class='center' style=" style="text-align:center;">
									<a href="javascript:void(0);" onclick="query(${var.activityId})">兑换记录</a>
								</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function query(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>acitity/receivedHistory?activityId='+id;
			 diag.Width = 1400;
			 diag.Height = 700;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
