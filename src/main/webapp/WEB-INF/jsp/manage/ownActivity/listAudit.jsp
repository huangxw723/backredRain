...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 查询  -->
			<form action="ownActivity/listAudit.do" method="post" name="usersForm" id="usersForm">
				...
				<c:choose>
					<c:when test="${not empty list}">
						...
						<td class='center' style="width: 30px;">
                            <label>
                              <input type='checkbox' name='ids' value="${var.ownActivityId}" />
                              <span class="lbl"></span>
                            </label>
                          </td>
								...
								<td class='center'  style="text-align:center;">
									<a href="javascript:void(0);" onclick="activityStatus(${var.ownActivityId})">审批</a>
								</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function activityStatus(id){
			...
			diag.Drag=true;
			diag.Title ="查看详情";
			diag.URL='<%=basePath%>ownActivity/auditStatus?status=3&ownActivityId='+id; 
			diag.Width = 900;
	        diag.Height = 700;
		        ...
		        if(str==''){
		        	...
		        	if(msg == '确定要审批通过选中的数据吗?'){
						top.jzts();
						  $.ajax({
				   			    type : "post",
				   		        url : "ownActivity/updateAuditStatus",
				   		        dataType : "json",
				   				async: false,
				   				data : {
				   					ids: str
				   		        ...
				   		        success : function (data) {
				   		        	if("0"==data.status){
				   		        		...
				   		        		if(""==errIds){
					   		        		setTimeout(window.location.href="ownActivity/listAudit",100);
					   		        		return;
				   		        		}else{
