...
<script type="text/javascript">
	...
	function save(){
        ...
        var shopId = $("#shopId").val();
				$.ajax({
					type: "POST",
					url: '<%=basePath%>ownActivity/isExist.do',
					data: {startTime:startTime,shopId:shopId},
					dataType:'json',
					cache: false,
		...
		if($(".probability").length % 2 != 0){
			bootbox.alert("设置的奖品份数必须为偶数！");
			return;
		}else if($(".probability").length < 6){
			bootbox.alert("设置的奖品份数不能小于6！");
			return;
		} 
...
<script type="text/javascript">
	jQuery(document).ready(function(){
		...
		$("#activityName").blur(function(){
				var activityName = $('#activityName').val();
				if(!validateUtils.isEmpty(activityName)) {
					if (activityName.length < 2 || activityName.length > 10) {
						$("#activityName").tips({
							side: 3,
							msg: '2~10个字',
			...
			$("#perCount").blur(function(){
				var perCount = $('#perCount').val();
				if(!validateUtils.isEmpty(perCount)) {
					if (perCount < 1) {
						$("#perCount").tips({
							side: 3,
							msg: '不能小于1次',
...
<body>																				
  <form action="ownActivity/save" name="form" id="form" method="post" enctype="multipart/form-data">
  		...
  		<%-- <input type="hidden" name="prizeConfigId" value="${prizeConfigId }" id="prizeConfigId">  --%>
    ...
    <div id="zhongxin">
      <table id="table_report" class="table table-striped table-bordered table-hover">
					  ...
					  <c:when test="${ not empty shopAll}">
						  <select class="chzn-select"  name="shopId" id="shopId" style="width: 383px; vertical-align:top;" >
							  <c:forEach items="${shopAll}" var="var"  >
								  <option value="${var.shopId}" >${var.name}</option>
							  </c:forEach>
						  </select>
					  </c:when>
					  <c:otherwise>
						  <select class="chzn-select"   name="shopId" id="shopId" style="width: 383px; vertical-align:top;">
							  <option value="${asShop.shopId}" >${asShop.name}</option>
						  </select>
					  </c:otherwise>
				  </c:choose>
			  </td>
        ...
        <tr>
        	...
        	<td><input type="text" name="activityName" value="${asOwnActivity.activityName }" style="width: 370px; vertical-align:top;"  placeholder="请输入活动名称" id="activityName"></td>
        </tr>

        <tr>
        	<td width="150px">每天/每人参与次数</td>
        	<td><input type="number" name="perCount" min="1" value="${asOwnActivity.perCount }" style="width: 370px; vertical-align:top;"  placeholder="每天/每人参与次数" id="perCount" onkeyup="validateUtils.replaceNotNumEmpty(this);"></td>
        </tr>
         <tr>
      ...
      <div id="box" >
	        ...
	        <div class="gift">
	        	<c:forEach begin="0" end="5">
		        		<div class="body">
			        		<font style="font-size: 18px">中奖几率(%)：</font><input type="number" name="probability"  class="probability"  placeholder="请输入中奖几率" >
			        		<input type="text" name="prizeConfigId"  class="prizeConfigId" value="请选择礼品" giftId="" >
        	...
        	<div class="end">
        		<a href="javascript:;" class="add">添加礼品</a>&nbsp;&nbsp;
        		<a href="javascript:;" class="onAdd" >批量添加礼品</a>&nbsp;&nbsp;
     ...
     <div class="center" colspan="2">
            <a class="btn btn-mini btn-primary" onclick="save();">添加活动</a>&nbsp;&nbsp;&nbsp;&nbsp;
