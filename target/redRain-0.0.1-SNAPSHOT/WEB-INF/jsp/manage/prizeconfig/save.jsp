...
<script type="text/javascript">
	...
	function save(){
		...
		if(categoryName != "积分" && "${action}" == 1 || categoryName != "积分" && "${action}" == 2  ){
...
<body>
	...
	<table id="table_report" class="table table-striped table-bordered table-hover" style="overflow:scroll;">
		<tr height="45px">
			...
			<td>总发放数:</td>
			<td><input <c:if test="${action == 4 || pd.status == 1 || pd.status == 2}">disabled="disabled"</c:if>placeholder="1~9999" type="number" min="1" max="9999" name="counts" id="counts" value="${pd.counts}" style="width: 370px" onkeyup="validateUtils.replaceNotNumEmpty(this);"  /></td>
		</tr>
		<tr height="45px">
			<td>所属商户:</td>
			<td>
			...
			<c:choose>
				<c:when test="${action != 1 &&pd.status == 1 &&pd.status == 2}">
					<td>剩余数量:</td>
					<td><input <c:if test="${action == 4}">disabled="disabled"</c:if>placeholder="0~9999" type="number" min="0" max="9999" name="surplusCounts" id="surplusCounts" value="${pd.surplusCounts}" style="width: 370px" onkeyup="validateUtils.replaceNotNumEmpty(this);" /></td>
				</c:when>
			...
			<td>
				<select class="chzn-select" <c:if test="${action == 4 || pd.status == 1 || pd.status == 2}">disabled="disabled"</c:if> data-placeholder="请选择" id="categoryId" name="categoryId" style="width: 383px; vertical-align:top;">
					...
					<c:if test="${'积分' eq pd.prizeTypeName }">
						<td id="td3">发放积分:</td>
						<td id="td4"><input <c:if test="${action == 4 || pd.status == 1 || pd.status == 2}">disabled="disabled"</c:if> placeholder="1~9999" type="number" min="1" name="price" id="price" value="${pd.price}" style="width: 370px" onkeyup="validateUtils.replaceNotNumEmpty(this);" />点</td>
					</c:if>
		</tr>
		...
		<tr height="45px">
			...
			<td><div class="row-fluid input-append date">
									<input <c:if test="${action == 4 || '积分' eq pd.prizeTypeName}">disabled="disabled"</c:if> name="startTime" type="text" data-date-format="yyyy-mm-dd hh:ii:00" id="startTime" readonly="readonly" placeholder="开始时间" class="span10 date-picker" value="${pd.startTime}">
									<span class="add-on"><i class="icon-calendar"></i></span>
								</div></td>
			<td>结束时间:</td>
			<td><div class="row-fluid input-append date">
									<input <c:if test="${action == 4 || '积分' eq pd.prizeTypeName}">disabled="disabled"</c:if> name="endTime" type="text" data-date-format="yyyy-mm-dd hh:ii:00" id="endTime" readonly="readonly" placeholder="结束时间" class="span10 date-picker" value="${pd.endTime}" >
									<span class="add-on"><i class="icon-calendar"></i></span>
								</div></td>
		</tr>
		<tr height="45px">
