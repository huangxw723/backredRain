...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="prizeconfig/shakeListAll.do" method="post" name="usersForm" id="usersForm">
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<!-- <th style="text-align:center;">审核理由</th> -->
					</tr>
				</thead>
					...
					<c:when test="${not empty list}">
						<c:forEach items="${list}" var="var" varStatus="vs">
							...
							<tr>
								...
								<td class='center' style="text-align:center;" id="p">
									${var.shopName}&nbsp;
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function toExcel(){
			...
			window.location.href='<%=basePath%>prizeconfig/shakeExcel.do?categoryId='+categoryId+'&status='+status+'&isJackpot='+isJackpot+'&keyWord='+keyWord+'&showCount='+showCount+'&currentPage='+currentPage;
		...
		function addPrize(){
					...
					form.find('#showCount').attr('name','showCount');
					 ...
					 diag.Drag=true;
					 diag.Title ="添加礼品";
					 diag.URL = '<%=basePath%>prizeconfig/shakePage?action=1&activityType='+"${activityType}"+"&isJackpot="+"${pd.isJackpot}";
					 diag.Width = 1150;
					 diag.Height = 750;
		...
		function edit(id){
			...
			var form = $("#usersForm");
			var parems = 'action=2'+ '&prizeConfigId='+id+'&activityType='+"${activityType}"+"&isJackpot="+"${pd.isJackpot}" ;
			form.find('#currentPage').attr('name','currentPage');
			form.find('#showCount').attr('name','showCount');
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>prizeconfig/shakePage.do?'+parems;
			 diag.Width = 1150;
			 diag.Height = 750;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
