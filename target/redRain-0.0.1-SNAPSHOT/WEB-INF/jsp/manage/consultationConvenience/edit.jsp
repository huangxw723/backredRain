...
<body>
	<form class="form-horizontal" id="form" name="form"
		action="consultationConvenience/saveEdit.do" method="post"
		enctype="multipart/form-data">
		<input type="hidden" name="consultationConvenienceId"
			id="consultationConvenienceId"
			value="${obj.consultationConvenienceId }" /> 
		<input type="hidden"
			name="visitCount" id="visitCount" value="${obj.visitCount }" /> <input
			type="hidden" name="publicTime" id="publicTime"
			value="${obj.publicTime }" /> 
		...
		<div id="zhongxin">
			<table id="table_report"
				...
				<tr>
					<th>资讯类型:</th>
					<td><input type="text" value="便民" disabled="disabled">
					 <select  name="type" id="type" style="vertical-align:top;width: 100px">					 				                                                                   
                        <option value="1"  <c:if test="${obj.type eq '1'}">selected="selected"</c:if>>交通指南</option>	
					 	<option value="2"  <c:if test="${obj.type eq '2'}">selected="selected"</c:if>>免费公园</option>	                                                   							
					</select>
					</td>
				</tr>
				<tr>
					<th>城市:</th>
					<td style="line-height: 32px"><span
						style="float: left; margin-top: 5px"> 省份:<select
							name="provinceId" id="provinceId" data-placeholder="请选择省份"
							style="vertical-align: top; width: 115px">
								<option value="">全部</option>
								<c:forEach items="${provinceList}" var="province">
									<option value="${province.provinceId }"
										<c:if test="${province.provinceId == obj.provinceId}">selected</c:if>>${province.provinceName }</option>
								</c:forEach>
						</select>
					</span> <span style="float: left; margin-left: 5px; margin-top: 5px">
							城市:<select name="cityId" id="cityId" data-placeholder="请选择城市"
							style="vertical-align: top; width: 115px;">
						</select>
					</span> <span style="float: left; margin-left: 5px; margin-top: 5px">
							区域:<select name="areaId" id="areaId" data-placeholder="请选择县区"
							style="vertical-align: top; width: 115px">
						</select>
					</span></td>
				</tr>
				...
				<tr>
					...
					<td><input type="text" style="width: 90%;" name="title"
						id="title" value="${obj.title}" placeholder="这里输入标题" title="标题" />
					</td>
				</tr>
				...
				<tr>
					...
					<td><input type="text" name="phone" id="phone"
						value="${obj.phone }" placeholder="请输入联系电话" style="width: 400px"></td>
				</tr>

				<tr>
					<th>开放时间:</th>
					<td><input type="text" name="openTime" id="openTime"
						value="${obj.openTime}" placeholder="请输入开放时间，开始时间和结束时间请以英文分号分割"
						style="width: 400px"> <font color="red">(如:周一到周五：9:00-21:00;周六到周日：8:00-22:00
							)</font></td>
				</tr>
				...
				<tr>
					<th>封面图:</th>
					<td><c:if
							test="${obj == null || obj.coverImg == '' || obj.coverImg == null }">
							...
							test="${obj != null && obj.coverImg != '' && obj.coverImg != null }">
							<a id="imgBox" class="has" href="<%=httpUrl%>${obj.coverImg}"
								title="${obj.coverImg}"> <img
								src="<%=httpUrl%>${obj.coverImg}" alt="${obj.coverImg}"
								width="750" height="666" id="imgHref" />
							</a>
				...
				<tr>
					<th>线路图:</th>
					<td><c:if
							test="${obj == null || obj.lineImg == '' || obj.lineImg == null }">
							...
							test="${obj != null && obj.lineImg != '' && obj.lineImg != null }">
							<a id="imgBox2" class="has" href="<%=httpUrl%>${obj.lineImg}"
								title="${obj.lineImg}"> <img
								src="<%=httpUrl%>${obj.lineImg}" alt="${obj.lineImg}"
								width="750" height="666" id="lineImgs" />
							</a>
							<input type="button" class="btn btn-mini btn-danger" value="更改"
								onclick="changeImg2()" />
				...
				<%-- <tr>
					<td>地址:</td>
					<td><input type="text" style="width: 370px;" name="address"
						id="address" value="${obj.address}" placeholder="这里输入地址"
						title="地址" /> <input type="button" value="地图上找"
						onClick="sear(document.getElementById('address').value);" /> <br />
						<div style="width: 520px; height: 340px; border: 1px solid gray"
							id="container"></div> <br /> 经纬度：<input id="lonlat"
						name="lonlat" type="text" disabled="disabled" value="${latlng}">
					</td>
				</tr> --%>
	          ...
	          <td>
				...
				<tr>
					<td class="center" colspan="2"><a
						class="btn btn-mini btn-primary" onclick="save(2);">立即发布</a> <a
						class="btn btn-mini btn-primary" onclick="save(1);">保存为草稿</a></td>
				</tr>
			</table>
		</div>
			...
			if ($("#provinceId").val()) {
				...
				$.each(JSON.parse(cityList), function(i, list) {
					...
					if (parentId == provinceId) {
						...
						var label = $("<option value='"+value+"'>" + option
								+ "</option>");
