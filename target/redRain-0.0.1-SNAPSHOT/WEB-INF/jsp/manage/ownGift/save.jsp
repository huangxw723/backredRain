	...
	$(function() {
			...
			$("#categoryId").change(function(){
			     ...
			     if(value == "折扣券"){
			    	 $("#td3").html("折扣力度:");
			    	 $("#td4").html("");
			    	 $("#td4").html('<input placeholder="1.0~9.9" type="number" min="1.0" max="9.9" step="0.1" name="price" id="price"  style="width: 370px" />折');
			     }else if(value == "代金券"){
			    	 $("#td3").html("优惠额度:");
			    	 $("#td4").html("");
			    	 $("#td4").html('<input placeholder="0.01~9999.99" type="number" min="0.01" max="9999.99" name="price" id="vouchers"  style="width: 370px" />元');
			     }else if(value == "礼品券"){
			    	 $("#td3").html("");
			    	 $("#td4").html("");
...
<body>
	...
	<form action="ownGift/save" name="usersForm" id="usersForm" method="post" enctype="multipart/form-data">
	<div id="zhongxin" >
	<table id="table_report" class="table table-striped table-bordered table-hover" style="overflow:scroll;">
					...
					<c:when test="${ not empty shopAll}">
						<select class="chzn-select"  name="shopId" id="shopId" style="width: 383px; vertical-align:top;" >
							<c:forEach items="${shopAll}" var="var"  >
								<option value="${var.shopId}" storeId="${var.storeId}" >${var.name}</option>
							</c:forEach>
						</select>
					</c:when>
					<c:otherwise>
						<select class="chzn-select"   name="shopId" id="shopId" style="width: 383px; vertical-align:top;">
							<option value="${asShop.shopId}" storeId="${asShop.storeId}" >${asShop.name}</option>
						</select>
					</c:otherwise>
				</c:choose>
			</td>
		...
		<tr height="20px">
			<td>礼品名称:</td>
			<td><input placeholder="2~10个字" type="text" name="name" id="name" style="width: 370px" /></td>
			<td>总发放数</td>
			<td><input placeholder="1~9999" min="1" max="9999" type="number" name="counts"  id="counts" style="width: 370px" onkeyup="validateUtils.replaceNotNumEmpty(this);" /></td>
		</tr>
		
		<tr height="20px">
			<td>礼品类型:</td>
			<td>
				<select class="chzn-select" data-placeholder="请选择"  name="categoryId" id="categoryId" style="width: 383px; vertical-align:top;">
			...
			<td id="td3">折扣力度</td>
			<td id="td4"><input placeholder="1~9.9折" type="number" name="price" min="1" id="price" style="width: 370px" /></td>
		</tr> 
		
		<tr>
			<td>开始时间:</td>
			<td><div class="row-fluid input-append date">
					<input name="startTime" type="text" data-date-format="yyyy-mm-dd hh:ii:00" id="startTime" readonly="readonly" placeholder="开始时间" class="span10 date-picker" value="${pd.startTime}">
					<span class="add-on"><i class="icon-calendar"></i></span>
				</div></td>
			<td>结束时间:</td>
			<td><div class="row-fluid input-append date">
					<input  name="endTime" type="text" data-date-format="yyyy-mm-dd hh:ii:00" id="endTime" readonly="readonly" placeholder="结束时间" class="span10 date-picker" value="${pd.endTime}" >
					<span class="add-on"><i class="icon-calendar"></i></span>
				</div></td>
		</tr>
		<tr height="45px">
			<td>生效时间:</td>
			<td><div class="row-fluid input-append date">
				<input name="activeTime" type="text" data-date-format="yyyy-mm-dd hh:ii:00" id="activeTime" readonly="readonly" placeholder="生效时间" class="span10 date-picker" value="${pd.activeTime}">
				<span class="add-on"><i class="icon-calendar"></i></span>
			</div></td>
			<td>失效时间:</td>
			<td><div class="row-fluid input-append date">
				<input name="inactiveTime" type="text" data-date-format="yyyy-mm-dd hh:ii:00" id="inactiveTime" readonly="readonly" placeholder="失效时间" class="span10 date-picker" value="${pd.inactiveTime}" >
				<span class="add-on"><i class="icon-calendar"></i></span>
			</div></td>
		</tr>
		...
		<tr>
			...
			<td>
	            ...
	            <input id="url" name="logoUrl" type="hidden" />
            </td>
            
			<td>描述:</td>
			<td><textarea  placeholder="0~200个字" name="descp" id="descp" style="width: 400px;resize:none;height:200px;" ></textarea></td>
		</tr>
