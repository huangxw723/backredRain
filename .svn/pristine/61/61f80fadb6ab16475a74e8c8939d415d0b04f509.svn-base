...
<body>
	<div id="page-content" class="clearfix">
		...
		<div class="row-fluid">
			...
			<form class="form-horizontal" id="form" action="yp/admin/template/saveImg.do" method="post" enctype="multipart/form-data">
						...
						<c:choose>
							<c:when test="${image != null}">
								...
								<span class="help-inline" style="text-align: center;">
									...
									<a href="javascript:return false" onclick="delImg(${businessModuleId})">删除</a>
								</span>
								<div style="float: left;">
									<img id="image1" alt="" src="<%=httpUrl%>${image}" style="max-width:350px;"/>
								</div>
							</c:when>
							<c:otherwise>
		...
		function delImg(businessModuleId){
			if(confirm("确认删除图片吗?")){
				$.ajax({
						type: "POST",
						url: '<%=basePath%>yp/admin/template/delBackgroundImg.do',
		...
		$(document).ready(function(){
			...
			$("#save").on("click",function(){
				...
				if(suffix != "jpg" && suffix != "png" && suffix != "jpeg" && suffix != "JPG" && suffix != "PNG"){
					alert("请选择图片文件");
					return ;
				}
				hasClick = true;
