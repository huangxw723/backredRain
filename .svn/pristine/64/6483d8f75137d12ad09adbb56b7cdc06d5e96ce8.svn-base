...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="coupon/myCoupons.do" method="post" name="" id="">
				<table>
							<tr>
						...
						<c:forEach items="${list}" var="coup" varStatus="vs">
							...
							<tr>
								...
								<c:choose>
									...
									<c:when test="${coup.status ==1}">
										<a href="javascript:void(0);" onclick="disable(${coup.couponId},'${coup.name}')">禁用</a>
									</c:when>
									<c:when test="${coup.status ==2}">
										<a href="javascript:void(0);" onclick="enable(${coup.couponId},'${coup.name}')">启用</a>
									</c:when>
								</c:choose>
									<c:if test="${coup.status ==0}">
										<a href="javascript:void(0);" onclick="edit(${coup.couponId})">编辑</a>
									</c:if>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>coupon/toAddCoupon.do';
			 diag.Width = 700;
			 diag.Height = 500;
		...
		function edit(couponId){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>coupon/toEditCoupon.do?couponId='+couponId;
			 diag.Width = 700;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		//禁用
		function disable(couponId,name){
			if(confirm("确定禁用优惠券"+name+"吗?")){
				$.ajax({
					type: "POST",
		...
		function enable(couponId,name){
			if(confirm("确定启用优惠券"+name+"吗?")){
				$.ajax({
					type: "POST",
					url: '<%=basePath%>coupon/enable.do',
			    	data: {couponId:couponId},
					dataType:'json',
					//beforeSend: validateData,
					cache: false,
