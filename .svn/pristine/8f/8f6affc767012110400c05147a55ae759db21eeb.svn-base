...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 查询  -->
			<form action="shopowner/getShopOperatListAudit.do" method="post" name="usersForm" id="usersForm">
			<input type="hidden" id="shopId" value="${shopId}"/>
			<input type="hidden" id="shopOwnerInfoId" value="${shopOwnerInfoId}"/>
			<table>
					<tr>
						...
						<span>
						 ...
						 <span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" placeholder="请输入：商户名称/用户名/申请人"
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function updateAuditStatus(shopId,shopOwnerId){
			 ...
			 diag.Drag=true;
			 diag.Title ="审核处理";
			 diag.URL = '<%=basePath%>shopowner/toshopInfoListPage.do?action=1&status=3&shopId='+shopId+'&shopOwnerId='+shopOwnerId;
			 diag.Width = 1500;
			 diag.Height = 800;
						  ...
						  $.ajax({
				   		        ...
				   		        success : function (data) {
				   		        	if("0"==data.status){
				   		        		...
				   		        		if(""==errIds){
					   		        		setTimeout(window.location.href="shopowner/getShopOperatListAudit",100);
					   		        		return;
				   		        		}else{
