...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<form action="bigWheel/receiveHistory.do" method="post" name="usersForm" id="usersForm">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function ReceivePrize(){
					...
					form.find('#showCount').attr('name','showCount');
					 ...
					 diag.Drag=true;
					 diag.Title ="兑换中心";
					 diag.URL = '<%=basePath%>bigWheel/toReceive.do';
					 diag.Width = 470;
					 diag.Height = 260;
		...
		function receive(name,sn) {
			bootbox.confirm("确定要兑换【"+name+" - ("+sn+")】吗?",function(result){
				if(result) {
					$.ajax({
						type: "POST",
						url: '<%=basePath%>bigWheel/editReceive.do',
						data: {sn:sn},
						dataType:'json',
						cache: false,
        ...
        function queryDetail(Id){	
			...
			form.find('#showCount').attr('name','showCount');
			 ...
			 diag.Drag=true;
			 diag.Title ="礼品详情";
			 diag.URL = "<%=basePath%>bigWheel/toPrizeDetail.do?id="+Id;
			 diag.Width = 750;
			 diag.Height = 630;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
