    ...
    <div id="page-content" class="clearfix">
        ...
        <div class="row-fluid">
            ...
            <div class="row-fluid">
                ...
                <!-- 查询  -->
                <form action="cate/cateList.do" method="post" name="usersForm" id="usersForm">   
                    ...
                    <table>
                        <tr>
                            <td>
								...
								<select name="cityId" id="cityId" data-placeholder="请选择城市" class="chzn-select" style="vertical-align:top;width: 115px" > 
							  ...
							  <span class="input-icon">
								店铺名称:&nbsp;<input autocomplete="off" id="nav-search-input" type="text" name="name" value="${food.name}" placeholder="请输入关键词搜索" style="width:230px;"/>
                            ...
                            <c:forEach items="${list}" var="var" varStatus="vs">
                                ...
                                <tr>
                                    <td class='center' style="width: 30px;">
                                        <label>
                                            <input type='checkbox' name='ids' value="${var.consultationFoodId}"/>                                           
                                            <span class="lbl"></span>
                                        </label>
                                    </td>
                            	    ...
                            	    <td class='center'  style="text-align:center;">
                                        <a href="javascript:void(0);" onclick="preview(${var.consultationFoodId},${var.typeId})">预览</a>
                                        <a href="javascript:void(0);" onclick="edit(${var.consultationFoodId})">编辑</a>
                                        <a href="javascript:void(0);" onclick="editFoodMeal(${var.consultationFoodId},${var.typeId})">编辑菜式</a>
                                        <c:choose>
                                            <c:when test="${var.status ==1 || var.status ==3}">
                                                <a href="javascript:void(0);" onclick="status(${var.consultationFoodId},2)">发布</a>
                                            </c:when>
                                            <c:otherwise>
                                                <a href="javascript:void(0);" onclick="status(${var.consultationFoodId},3)">下架</a>
                                            </c:otherwise>
                                        </c:choose>
                                        <a href="javascript:void(0);" onclick="del(${var.consultationFoodId},${var.typeId})">删除</a>
                                    </td>
                                </tr> 
...
<script type="text/javascript">
   ...
   function toExcel(){
	   top.jzts();
		...
		diag.Drag=true;
		diag.Title ="excel导入";
		diag.URL = '<%=basePath%>cate/toExcel';
		diag.Width = 600;
		diag.Height = 200;
    ...
    //预览
	function preview(id,typeId){
		
		top.jzts();
		var diag = new top.Dialog();
		diag.Drag=true;
		diag.Title ="美食资讯";
		diag.URL='<%=basePath%>cate/preview?id='+id+'&typeId='+typeId; 
		diag.Width = 700;
		diag.Height = 800;
		diag.CancelEvent = function(){ //关闭事件
			diag.close();
	...
	function add(){
		...
		diag.Drag=true;
		diag.Title ="新增";
		diag.URL = '<%=basePath%>cate/cateRelease';
		diag.Width = 900;
		diag.Height = 700;
		diag.CancelEvent = function(){ //关闭事件
			diag.close();
    ...
    function edit(id){
       // top.jzts();
        ...
        diag.Drag=true;
        diag.Title ="编辑";
        diag.URL = '<%=basePath%>cate/toEdit.do?id='+id;
        diag.Width = 900;
        diag.Height = 700;
        diag.CancelEvent = function(){ //关闭事件
            diag.close(); 
    ...
    function editFoodMeal(id,typeId){
        ...
        diag.Drag=true;
        diag.Title ="编辑";
        diag.URL = '<%=basePath%>cate/toEditMeal.do?action=2&id='+id+'&typeId='+typeId;
        diag.Width = 1000;
        diag.Height = 600;
        diag.CancelEvent = function(){ //关闭事件
            diag.close();
	...
	function status(id,status){
		$.get("cate/editStatus",{consultationFoodId:id,status:status},function(data){
	...
	function del(id,typeId){
		...
		$.get("cate/delete",{consultationFoodId:id,typeId:typeId},function(data){
