...
function TabView(f) {
	...
	var c = '<table class="tab_item"  id="{0}" border="0" cellpadding="0" cellspacing="0"><tr><td class="tab_item1"></td><td style="max-width: 80px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" title="{2}" class="tab_item2 tab_title">{1}</td><td class="tab_item2"><div class="tab_close"></div></td><td class="tab_item3"></td></tr></table>';
	...
	if (g.position == "bottom") {
		c = '<table class="tab_item_bottom"  id="{0}" border="0" cellpadding="0" cellspacing="0"><tr><td class="tab_item1_bottom"></td><td style="max-width: 80px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" title="{2}" class="tab_item2_bottom tab_title">{1}</td><td class="tab_item2_bottom"><div class="tab_close tab_close_bottom"></div></td><td class="tab_item3_bottom"></td></tr></table>';
		...
		if (a.current == null || a.current != this) {
			$(p).mouseover(function() {
				...
				k.removeClass("tab_item3_mouseover" + b)
					...
					function() {
						if (a.current != null) {
							$(a.current).find(".tab_item1" + b).removeClass(
									"tab_item1_selected" + b);
							$(a.current).find(".tab_item2" + b).removeClass(
									"tab_item2_selected" + b);
							$(a.current).find(".tab_item3" + b).removeClass(
									"tab_item3_selected" + b);
							$(a.current).find(".tab_close").addClass(
						...
						a.current = this;
						$(a.current).find(".tab_close").removeClass(
								"tab_close_noselected");
...
//			if (this.tabContainer.find(".tab_item").length > g.maxtabnum) {
//				alert("以超过最大tab数量,请关闭其他tab!");
//				isExist = true;
//			} else {
				var oldtitle = m.title;
				if (m.title.length > 10) {
					m.title = m.title.substring(0, 8) + ".."
				}
				if (m.title.length < 4) {
					m.title = "&nbsp;&nbsp;" + m.title + "&nbsp;"
				}
				...
				$("#" + this.pid).append(k);
				var l = c.replace("{0}", m.id).replace("{1}", m.title).replace("{2}", oldtitle);
...
//			}
	...
	this.close = function(k) {
		$("#" + k).find(".tab_close").trigger("click");
	};
	this.init()
};