...
<head>
	...
	<script type="text/javascript">
		...
		function save(){
			...
			}else if($('#rangeType option:selected').val()==5){
				itemId=$("#shop").val()
...
<form action="link/activityLink/addCouponLink.do" name="usersForm" id="usersForm" method="post" enctype="multipart/form-data">
								...
								商户:
                                    <select class="chzn-select" name="shop" id="shop" data-placeholder="请选择商户" style="vertical-align:top;width: 115px">
                                      ...
                                      <c:forEach items="${shopOwnerInfosList}" var="shopOwnerInfo">
										  <option value="${shopOwnerInfo.shopId}" >${shopOwnerInfo.shopName }</option>
									  </c:forEach>
                                    </select>
							    </span>
                            </span>
