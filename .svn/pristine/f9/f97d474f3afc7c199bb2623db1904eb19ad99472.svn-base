    ...
    <div id="page-content" class="clearfix">
        ...
        <div class="row-fluid">
            ...
            <div class="row-fluid">
                ...
                <!-- 查询  -->
                <form action="spots/cateList.do" method="post" name="usersForm" id="usersForm"> 
                            ...
                            <c:forEach items="${list}" var="var" varStatus="vs">
                                ...
                                <tr>
                                    <td class='center' style="width: 30px;">
                                        <label>
                                            <input type='checkbox' name='ids' value="${var.consultationSpotsId}"/>                                           
                                            <span class="lbl"></span>
                                        </label>
                                    </td>
                            	    ...
                            	    <td class='center'  style="text-align:center;">
                            	    	<a href="javascript:void(0);" onclick="preview(${var.consultationSpotsId},${var.typeId})">预览</a>
                                        <a href="javascript:void(0);" onclick="edit(${var.consultationSpotsId})">编辑</a>
                                        <a href="javascript:void(0);" onclick="editFoodMeal(${var.consultationSpotsId},${var.typeId})">编辑宣传图</a>
                                        <c:choose>
                                            <c:when test="${var.status ==1 || var.status ==3}">
                                                <a href="javascript:void(0);" onclick="status(${var.consultationSpotsId},2)">发布</a>
                                            </c:when>
                                            <c:otherwise>
                                                <a href="javascript:void(0);" onclick="status(${var.consultationSpotsId},3)">下架</a>
                                            </c:otherwise>
                                        </c:choose>
                                        <a href="javascript:void(0);" onclick="del(${var.consultationSpotsId},${var.typeId})">删除</a>
                                    </td>
                                </tr> 
...
<script type="text/javascript">
	...
	function preview(id,typeId){
		...
		diag.Drag=true;
		diag.Title ="景点资讯";
		diag.URL='<%=basePath%>spots/preview?id='+id+'&typeId='+typeId; 
		diag.Width = 700;
		diag.Height = 800;
	...
	function add(){
		...
		diag.Drag=true;
		diag.Title ="新增";
		diag.URL = '<%=basePath%>spots/spotsRelease';
		diag.Width = 900;
		diag.Height = 700;
		diag.CancelEvent = function(){ //关闭事件
			diag.close();
    ...
    function edit(id){
       // top.jzts();
        ...
        diag.Drag=true;
        diag.Title ="编辑";
        diag.URL = '<%=basePath%>spots/toEdit.do?id='+id;
        diag.Width = 900;
        diag.Height = 700;
        diag.CancelEvent = function(){ //关闭事件
            diag.close(); 
    ...
    function editFoodMeal(id,typeId){
        ...
        diag.Drag=true;
        diag.Title ="编辑";
        diag.URL = '<%=basePath%>spots/toEditMeal.do?action=2&id='+id+'&typeId='+typeId;
        diag.Width = 1000;
        diag.Height = 600;
        diag.CancelEvent = function(){ //关闭事件
            diag.close();
	...
	function status(id,status){
		$.get("spots/editStatus",{consultationSpotsId:id,status:status},function(data){
	...
	function del(id,typeId){
		if(confirm("确定删除吗?")){
		$.get("spots/delete",{consultationSpotsId:id,typeId:typeId},function(data){
        ...
        bootbox.confirm(msg, function(result) {
            if(result) {           
                ...
                if(str==''){
                    ...
                    if(msg == '确定要下架选中的数据吗?'){
                        top.jzts();                   
                        var url = "<%=basePath%>spots/offSaleAll.do?ids="+str;
            ...
            if(result) {
                ...
                if(str==''){
                    ...
                    if(msg == '确定要删除选中的数据吗?'){
                        ...
                        var url = "<%=basePath%>spots/deleteAll.do?ids="+str;
                        $.get(url,function(data){
                            if(typeof(nextPage) == "function") {
                              //  nextPage(${page.currentPage});
                            	location.reload()
