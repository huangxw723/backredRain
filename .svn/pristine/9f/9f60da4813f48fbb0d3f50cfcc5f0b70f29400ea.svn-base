...
<div class="container-fluid" id="main-container">
    <div id="page-content" class="clearfix">
        <div class="row-fluid">
            <div class="row-fluid">
                <!-- 检索  -->
                <form action="cooperation/cooperationList.do" method="post" name="Form" id="Form">
                                    ...
                                    <c:forEach items="${list}" var="var" varStatus="vs">
                                        <tr>
                                            <td class='center' style="width: 30px;">
                                                <label>
                                                    <input type='checkbox' name='ids' onclick="checkboxOnclick(this)" value="${var.cooperationId}"/>
                                                    <span class="lbl"></span>
                                                </label>
                                            </td>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
                                            <td class='center' style="text-align:center;">${var.unactiveId}</td>
                                            <td class='center' style="text-align:center;">${var.shop.name}</td>
                                             <td class='center' style="text-align:center;">
                                            	<c:if test="${var.shop.type eq 1}">
                                            		商场
                                            	</c:if>
                                            	<c:if test="${var.shop.type eq 2}">
                                            		商户
                                            	</c:if>
                                            </td>
                                            <%--<td class='center' style="text-align:center;">${var.shop.address}(${var.shop.lng},${var.shop.lat})</td>--%>
                                            <td class='center' style="text-align:center;">${var.shop.address}</td>
                                        </tr>
...
<script type="text/javascript">
     ...
     function add(){
         top.mainFrame.tab.add( {
             id :'addCooperation'+"${shopId}",
             title :"添加战略伙伴",
             url :'<%=basePath%>cooperation/addCooperation.do?shopId='+"${shopId}",
             isClosed :true,
            ...
            if (result) {
                ...
                if (str == '') {
                    ...
                    return;
                        ...
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>cooperation/deleteCooperationIds.do',
