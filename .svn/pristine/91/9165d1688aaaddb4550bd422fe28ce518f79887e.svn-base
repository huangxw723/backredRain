...
<body>
	...
	<form action="consultation/brand/updateBrand.do" name="usersForm" id="usersForm" method="post">
	...
	<c:if test="${action != null && action !=1 }">
	     <tr height="45px">
			<td>品牌名称:</td>
			<td><input  type="text" name="brandName" id="brandName" value="${asBrand.brandName}" style="width: 370px"></td>
		</tr>
    </c:if>
		<tr>
			<td>品牌logo:</td>
			...
			<td>
              <select class="chzn-select" name="ypBaseCategoryId" id="ypBaseCategoryId">
                  <c:forEach items="${ypBaseCategoryList}" var="ypBaseCategory">
                      <option value="${ypBaseCategory.id}" <c:if test="${ypBaseCategory.id == asBrand.ypBaseCategoryId}">selected</c:if>>${ypBaseCategory.name}</option>
                  </c:forEach>
              </select>
          </td>
		</tr>
		<tr height="45px">
			<td>品牌描述：</td>
			<td><input  type="text" name="brandDesc" id="brandDesc" value="${asBrand.brandDesc}" style="width: 370px" /></td>
		</tr>

		<tr height="45px">
			<td>品牌官网地址：</td>
			<td><input  type="text" name="siteUrl" id="siteUrl" value="${asBrand.siteUrl}" style="width: 370px" /></td>
		</tr>
		...
		<tr height="45px">
            ...
            <td>
                <select name="isShow" id="isShow">
                    <option value="Y" <c:if test="${asBrand.isShow=='Y'}">selected="selected"</c:if> >显示</option>
                    <option value="N" <c:if test="${asBrand.isShow=='N'}">selected="selected"</c:if>>不显示</option>
                </select>
            </td>
		</tr>
