...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<form action="ownActivity/ownActivityReceiveHistory.do" method="post" name="usersForm" id="usersForm">
					...
					<tr>
								<td>
									<span class="input-icon"> 
										<input autocomplete="off" id="nav-search-input" type="text" style="width: 240px;" name="keyWord" value="${pd.keyWord}" placeholder="兑换编号/礼品名称" />
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function ReceivePrize(){
					...
					form.find('#showCount').attr('name','showCount');
					 ...
					 diag.Drag=true;
					 diag.Title ="兑换中心";
					 diag.URL = '<%=basePath%>ownActivity/toReceive.do';
					 diag.Width = 470;
					 diag.Height = 260;
		...
		function receive(name,sn) {
			bootbox.confirm("确定要兑换【"+name+" - ("+sn+")】吗?",function(result){
				if(result) {
					$.ajax({
						type: "POST",
						url: '<%=basePath%>ownActivity/editReceive.do',
						data: {sn:sn},
						dataType:'json',
						cache: false,
        ...
        function queryDetail(Id){	
			...
			form.find('#showCount').attr('name','showCount');
			 ...
			 diag.Drag=true;
			 diag.Title ="礼品详情";
			 diag.URL = "<%=basePath%>ownActivity/toPrizeDetail.do?id="+Id;
			 diag.Width = 750;
			 diag.Height = 630;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
