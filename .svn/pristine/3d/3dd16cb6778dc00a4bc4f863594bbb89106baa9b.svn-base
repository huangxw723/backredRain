...
public class Page {
	 ...
	 * 设置分页
	 * @param list
	 * 第三方分页插件，需要加载依赖，如不需要，可直接删除此方法
	 */
	@SuppressWarnings("unchecked")
	public void setPage(@SuppressWarnings("rawtypes") List list){
		PageInfo<Object> pageInfo = new PageInfo<Object>(list);
		this.setTotalResult((int)pageInfo.getTotal());
		this.setEntityOrField(true);
	}
	...
	public String getPageStr() {
		...
		if (totalResult > 0) {
			...
			if (currentPage == 1) {
				...
				sb.append("	<li><input type=\"number\" min=\"1\" value=\"\" id=\"toGoPage\" style=\"width:50px;text-align:center;float:left\" placeholder=\"页码\"/></li>\n");
