...
<div class="container-fluid" id="main-container">
    ...
    <div id="page-content" class="clearfix">
        <div class="row-fluid">
            <div class="row-fluid">
                <!-- 查询-搜索  -->  
                <form action="<%=basePath%>consultationHotel/list.do" method="post" name="usersForm" id="usersForm">
                    <table>
                        <tr>
                            <td>
							<span>
                            ...
                            <span class="input-icon">
								酒店名称:&nbsp;<input autocomplete="off" id="nav-search-input" type="text" name="name" value="${obj.name}" placeholder="请输入关键词搜索" style="width:120px;"/>
							</span>
                            </td>
                            ...
                            <c:forEach items="${list}" var="var" varStatus="vs">
                                ...
                                <tr>
                                    <td class='center' style="width: 30px;">
                                        <label>
                                            <input type='checkbox' name='ids' value="${var.consultationTrafficId}" />
                                            <span class="lbl"></span>
                                        </label>
                                    </td>
                                    ...
                                    <td class='center' style="text-align:center;">${var.typeName}</td><!--类型  -->   
                                   <!-- <td class='center' style="text-align:center;">/</td> 二级分类   -->
                                    <td class='center' style="text-align:center;">${var.name}</td>  
                                    <td class='center' style="text-align:center;">${var.brandName}</td> 
                                    ...
                                    <td class='center'  style="text-align:center;">
                                        <a href="javascript:void(0);" onclick="editPreview(${var.consultationTrafficId})">预览</a>
                                        <a href="javascript:void(0);" onclick="edit(${var.consultationTrafficId})">编辑</a>
                                         <a href="javascript:void(0);" onclick="editFoodMeal(${var.consultationTrafficId},${var.typeId})">上传多张酒店实拍图</a>
                                        <c:choose>
                                            <c:when test="${var.status ==3 || var.status ==1}">
                                                <a href="javascript:void(0);" onclick="onSale(${var.consultationTrafficId})">发布</a>
                                            </c:when>
                                            <c:otherwise>
                                                <a href="javascript:void(0);" onclick="offSale(${var.consultationTrafficId})">下架</a>
                                            </c:otherwise>
                                        </c:choose>
                                        <a href="javascript:void(0);" onclick="del(${var.consultationTrafficId},${var.typeId})">删除</a>
                                    </td>
                                </tr>
                            </c:forEach>
                        </c:when>
...
<script type="text/javascript">
    ...
    function add(){
        ...
        diag.Drag=true;
        diag.Title ="发布";
        diag.URL = '<%=basePath%>consultationHotel/toAdd.do';
        diag.Width = 900;
        diag.Height = 700;
    ...
    //删除
    function del(Id,typeId){
        bootbox.confirm("确定要删除吗?", function(result) {
            if(result) {
                top.jzts();
                var url = "<%=basePath%>consultationHotel/delete.do?id="+Id+"&typeId="+typeId;
        ...
        bootbox.confirm(msg, function(result) {
            if(result) {
                ...
                if(str==''){
                    ...
                    if(msg == '确定要删除选中的数据吗?'){
                        top.jzts();
                        var url = "<%=basePath%>consultationHotel/deleteAll.do?ids="+str;
                        $.get(url,function(data){
                            if(typeof(nextPage) == "function") {
                                nextPage(${page.currentPage});
                            }
    ...
    function edit(id){
        ...
        diag.Drag=true;
        diag.Title ="编辑";
        diag.URL = '<%=basePath%>consultationHotel/toEdit.do?id='+id;
        diag.Width = 850;
        diag.Height = 700;
        diag.CancelEvent = function(){ //关闭事件
			diag.close();
    ...
    function editPreview(id){
        ...
        diag.Drag=true;
        diag.Title ="酒店预览";
        diag.URL = '<%=basePath%>consultationHotel/toEditPreview.do?id='+id;
        diag.Width = 850;
        diag.Height = 700;
        diag.CancelEvent = function(){ //关闭事件
			diag.close();
  ...
  //修改菜式
    function editFoodMeal(id,typeId){
        top.jzts();
        var diag = new top.Dialog();
        diag.Drag=true;
        diag.Title ="编辑";
        diag.URL = '<%=basePath%>consultationHotel/toEditMealHote.do?action=2&id='+id +"&typeId="+typeId;
        diag.Width = 1000;
        diag.Height = 600;
        diag.CancelEvent = function(){ //关闭事件
            diag.close();
    ...
    function onSale(id){
        if(confirm("确定发布吗?")){
            $.ajax({
                type: "POST",
                url: '<%=basePath%>consultationHotel/onSale.do',
                data: {id:id},
                dataType:'json',
                cache: false,
    ...
    function offSale(id){
        if(confirm("确定下架吗?")){
            $.ajax({
                type: "POST",
                url: '<%=basePath%>consultationHotel/offSale.do',
                data: {id:id},
                dataType:'json',
                cache: false,
                success: function(data){
        ...
        bootbox.confirm(msg, function(result) {
            if(result) {
                ...
                if(str==''){
                    ...
                    if(msg == '确定要下架选中的数据吗?'){
                        top.jzts();
                        var url = "<%=basePath%>consultationHotel/offSaleAll.do?ids="+str;
                        $.get(url,function(data){
                            if(typeof(nextPage) == "function") {
                                nextPage(${page.currentPage});
                            }
