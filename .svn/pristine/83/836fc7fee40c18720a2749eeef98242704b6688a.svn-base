...
<body>
	<form action="cate/cateSave.do" name="form" id="form" method="post"
		enctype="multipart/form-data">
		<input type="hidden" name="status" value=2 id="status"> <input
			type="hidden" id="latlng" name="latlng" value="">
		<div id="zhongxin">
			<table id="table_report"
				class="table table-striped table-bordered table-hover">
				<tr>
					<th>资讯类型:</th>
					<td><select class="chzn-select" name="typeId" id="typeId"
						class="chzn-select" style="vertical-align: top; width: 115px">
							<option value="${info.consultationTypeId}">${info.typeName }</option>
					</select></td>
				</tr>
				<tr>
					<th>城市:</th>
					<td><select name="provinceId" id="provinceId"
						onchange="select1()" class="chzn-select"
					...
					</select> <select name="cityId" id="cityId" onchange="select2()"
						class="chzn-select" style="vertical-align: top; width: 115px">
							<option value="">城市</option>

					</select> <select name="areaId" id="areaId" onchange="select3()"
						class="chzn-select" style="vertical-align: top; width: 115px">
							<option value="">区域</option>
					</select> <select name="businessDistrictId" id="businessId"
						class="chzn-select" style="vertical-align: top; width: 115px">
				...
				<tr>
					...
					<td><input type="text" style="width: 400px" name="name"
						id="title" placeholder="这里输入店铺名称" /></td>
				</tr>
				<tr>
					<th>品类:</th>
					<td><select name="ypBaseCategoryId" class="chzn-select" id="ypBaseCategoryId"
						style="vertical-align: top; width: 115px">
						 <c:choose>
				...
				<tr>
					...
					<td><input type="text" name="openTime" id="openTime"
						placeholder="请输入开放时间,开始时间和结束时间请以英文分号分割" style="width: 400px">
						<span style="color: red; width: 50px;" >例如:周一到周五：9:00-21:00;周六到周日：8:00-22:00 </span></td></td>
				</tr>
				<tr>
					<th>联系商家:</th>
					<td><input type="text" name="phone" id="phone"
						placeholder="请输入联系电话" style="width: 400px"></td>
				</tr>
				<tr>
				...
				<tr>
					<th>封面图:</th>
					<td><a id="imgBox" href="javascript:void(0)"
						style="display: none;"> <img src="" width="150" id="imgHref" />
					</a> <input type="file" id="fileInput" name="file" /> <c:if
							test="${store != null && store.coverImg != '' && store.coverImg != null }">
							<a id="imgBox" href="<%=httpUrl%>${store.coverImg}"> <img
								src="<%=httpUrl%>${store.coverImg}" width="150" id="imgHref" />
							</a>
							<input type="button" class="btn btn-mini btn-danger" value="更改"
								onclick="changeImg()" />
							<input type="file" id="fileInput" name="file"
								style="display: none;" />
						</c:if> <input id="url" name="coverImg" type="hidden" />
						<span style="color: red;font-size:15px;" >注:图片尺寸 宽:750 PX,高:666 PX</span></td>
				</tr>
				<tr>
					<th>特色菜图片</th>
					<td><input style="width: 75px;" type='file' id="fileInputs"
						onchange="selectImg(this)" name="imgs"> <span
						class="help-inline"> <a id="imgClean" style="display: none"
							href="javscript:void(0);" onclick="cleanImg()">清除图片</a>
					</span>
				...
				<tr>
					<td class="center" colspan="2"><a
						class="btn btn-mini btn-primary" onclick="save();">立即发布</a> <a
						class="btn btn-mini btn-danger" onclick="draft();">保存到草稿</a></td>
				</tr>
			</table>
		</div>
	...
	<script type="text/javascript">
		...
		function selectImg($this) {
			...
			$($this)
					.after(
							'<input style="width: 75px;" type="file" onchange="selectImg(this)" name="imgs">');
			...
			if (files) {
				for (var i = 0; i < files.length; i++) {
					...
					FR.onload = function(e) {
						...
						$("#images")
								.append(
										'<img class="imgs" style="max-height:100px;max-width:100px;margin-left: 5px;margin-top: 5px" id="img" src="'+result+'">');
						$("#imgClean").show();
					};
