...
public class OwnActivityService extends AsBaseService {
    ...
    @Autowired
    private  AsOwnActivityHistoryMapper asOwnActivityHistoryMapper;

    @Autowired
    private AsOwnActivityRelateMapper asOwnActivityRelateMapper;
     ...
     * @return
     */
    public Object toAddCount(BaseController c,Integer ownActivityId,Integer shopId,AsUser user) {
        initialized(c);
        if(user.getAccount() !=null && user.getAccount()!=""){
            List shopUser = new ArrayList();
            Map<String, Object> map = new HashMap<>();
            map.put("shopId",shopId);
            map.put("account",user.getAccount());
            map.put("ownActivityId",ownActivityId);
            Object object =asShopUserRelateMapper.queryAcoount(map);
            if(object !=null){
                shopUser.add(object);
                AsUser s= asUserMapper.queryByAccount(user.getAccount());
                if(s!=null){
                    addViewData("userId",s.getUserId());
                }
            }

            addViewData("shopUser",shopUser);
            addViewData("account",user.getAccount());
            addViewData("ownActivityId",ownActivityId);
           // return renderView(OWN_ACTICITY_PAHT + "addCount");
        }
        addViewData("shopId",shopId);
        addViewData("ownActivityId",ownActivityId);
        return renderView(OWN_ACTICITY_PAHT + "addCount");
    }
    ...
    public Object editCount(Integer userId,Integer count,Integer ownActivityId) {
        int result=0;
        int result2=0;
        AsOwnActivityRelate  ownActivityRelate = new AsOwnActivityRelate();
        AsOwnActivityHistory ownActivityHistory = new AsOwnActivityHistory();
        AsOwnActivityRelate owna= asOwnActivityRelateMapper.selectByOwnActivityId(ownActivityId,userId);
        if(userId !=null ){
            if(owna != null){
                if(owna.getUserId().equals(userId)){
                    owna.setCount(owna.getCount()+count);
                    result=asOwnActivityRelateMapper.updateByPrimaryKeySelective(owna);
                }
            }else {
                ownActivityRelate.setCount(count);
                ownActivityRelate.setOwnActivityId(ownActivityId);
                ownActivityRelate.setUserId(userId);
                result=asOwnActivityRelateMapper.insertSelective(ownActivityRelate);
            }

        }

        if (!getUser().getAs_shop_id().equals("null")) {
            ownActivityHistory.setShopId(Integer.valueOf(getCurrentUser().getAs_shop_id()));
        }else {
            ownActivityHistory.setShopId(0);
        }
        ownActivityHistory.setOwnActivityId(ownActivityId);
        ownActivityHistory.setCount(count);
        ownActivityHistory.setUserId(userId);
        ownActivityHistory.setSource(1);
        ownActivityHistory.setCreateTime(DateUtil.getTime());
        ownActivityHistory.setOperatorId(Integer.valueOf(getCurrentUser().getNewUserId()));
        ownActivityHistory.setOperator(getCurrentUser().getNAME());
        result2=asOwnActivityHistoryMapper.insertSelective(ownActivityHistory);
     ...
     * @return
     */
    public Object addCountHistoryList(BaseController c) {
        initialized(c);
        addField("shopId",getCurrentUser().getAs_shop_id());
        pageQueryModule(asOwnActivityHistoryMapper,"listAll",true);
        return renderView(OWN_ACTICITY_PAHT + "addCountHistory");
    }
