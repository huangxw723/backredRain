...
public class Watermark {
	 ...
	 *            图片全路径
	 */
	public static int[] setWatemark(String filePathImage, String imagePath) {
		int num[] = new int[2];
		// 文字水印
		...
		if (null != strIWATERM && !"".equals(strIWATERM)) {
			...
			if (strIW.length == 4) {
				if ("yes".equals(strIW[0])) {
					num = pressImage(PathUtil.getClasspath() + filePathImage + strIW[1], imagePath, Integer.parseInt(strIW[2]), Integer.parseInt(strIW[3]));
				}
			}
		}
		return num;
	 ...
	 *            --y坐标
	 */
	public final static int[] pressImage(String pressImg, String targetImg, int x, int y) {
		int[] num = new int[2];
		try {
			// 目标文件
			...
			num[0] = image.getWidth();
			num[1] = image.getHeight();
			// 水印文件
...
//			out.close();
	...
	public static int[] pressText(String pressText, String targetImg, String fontName, int fontStyle, Color color, int fontSize, int x, int y) {
		int[] num = new int[2];
		try {
			File _file = new File(targetImg);
			Image src = ImageIO.read(_file);
