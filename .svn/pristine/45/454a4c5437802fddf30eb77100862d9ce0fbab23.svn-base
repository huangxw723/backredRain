...
public class RandomCodeUtil {
	...
	public static String getPrizeSn() {
		...
		String temp = t.substring(t.length() - 5, t.length());
		String r = getNumber(5);
		return temp + r;
	}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 10000; i++) {
			String code = idToCode(i, 6);
			int id = codeToId(code);
			System.out.println(i + "__" + code + "__id: " + id);
			if (map.get(code) != null) {
				System.out.println("第 " + i + " 个重复了: " + map.get(code));
				break;
			}
			map.put(code, code);
		}
