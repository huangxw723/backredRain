...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="feedback/listAll.do" method="post" name="usersForm" id="usersForm">
					...
					<tr>
								<td>
									...
									<select name="status" id="status" style="width: 100px">
										...
										<option value="0" <c:if test="${pd.status eq '0'}">selected="selected"</c:if>>未处理</option>
										<option value="1" <c:if test="${pd.status eq '1'}">selected="selected"</c:if>>已处理</option>
									</select>
								</span>
								</td>
								<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th style="text-align:center;">意见反馈</th>
					...
					<c:when test="${not empty list}">
						<c:forEach items="${list}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="text-align:center;">${var.feedback}</td>
								<td class='center' style="text-align:center;">${var.contactNum}</td>
								...
								<td class='center'  style="text-align:center;">
									<c:if test="${var.status ==0}"><a href="javascript:void(0);" onclick="edit(${var.feedbackId})">处理</a></c:if>
									<c:if test="${var.status ==1}"><a href="javascript:void(0);" onclick="query(${var.feedbackId})">详情</a></c:if>
								</td>
							</tr>
						</c:forEach>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function query(id){
			...
			var form = $("#usersForm");
			var parems = 'action=4'+ '&feedbackId='+id ;
			form.find('#currentPage').attr('name','currentPage');
			form.find('#showCount').attr('name','showCount');
			 top.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="详情";
			 diag.URL = '<%=basePath%>feedback/page.do?'+parems;
			 diag.Width = 1150;
			 diag.Height = 510;
		...
		function edit(id){
			...
			var form = $("#usersForm");
			var parems = 'action=2'+ '&feedbackId='+id ;
			form.find('#currentPage').attr('name','currentPage');
			form.find('#showCount').attr('name','showCount');
			 top.jzts();
			 var diag = new top.Dialog();
